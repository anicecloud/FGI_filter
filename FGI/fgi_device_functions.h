...
__host__ __device__ float calculateRoad(uint32_T* window,uint32_T ws, const double* imageIn, uint32_T index, uint32_T elements, double* dist, uint32_T offset){
    double road_m = 0;
    for(uint i = 0; i < ws; i++){
		dist[i]=fabs(imageIn[index]-imageIn[window[i]+offset]);
    }
	//Sort both arrays by dist 
	bubbleSort(dist, window, ws);
...
__host__ __device__  void calcWindowIndexes(uint32_T* windowIndexes, uint8_T &windowRange, uint32_T rows, uint32_T cols, uint32_T windowSize , uint32_T range, uint32_T index ) {
